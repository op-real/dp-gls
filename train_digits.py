...
def train(args, model, ad_net,
          ...
          cov_mat, pseudo_target_label, class_weights, true_weights, wn_disc, wn_gen, optimizer_wnd, optimizer_wng):
    model.train()
    ...
    # source classifier loss
        if 'IW' in method:
            ...
            loss = torch.dot(
                ...
                weights.detach()) / torch.sum(weights.detach())
        else:
 ...
 # learn/update weights
    if 'WN' in method and epoch > start_epoch:
        train_wn(args, model, source_samples, target_samples, wn_disc, wn_gen, optimizer_wnd, optimizer_wng, nn.BCELoss(), nn.BCELoss(reduction='none'))
...
def main():
    ...
    if 'CDAN' in args.method:
        ad_net = network.AdversarialNetwork(model.output_num() * class_num, 500, sigmoid='WDANN' not in args.method)
        # wn_disc = network.WNDisc()
    else:
        ad_net = network.AdversarialNetwork(model.output_num(), 500, sigmoid='WDANN' not in args.method)
        wn_disc = network.WNDisc(model.output_num(), 500)
